    ...
    async move {
            ...
            client_references_by_server_component.into_iter()
        {
            ...
            for &client_reference_ty in client_reference_types.iter() {
                ...
                // Async only when *every* path to it is. Tests for an `Async` parent rather than
                // "not this segment": framework references are folded into the first segment but
                // parented on the entry group, and must stay eager.
                let is_async = !groups.is_empty()
                    && groups.iter().all(|&index| {
                        ...
                        matches!(
                            chunk_group_info_ref.chunk_groups.get_index(parent),
                            Some(ChunkGroup::Async(..))
                        )
                    });
                if is_async {
                    for &index in groups {
