...
export function createNavigationSeed(
  ...
  // see decodeTransportNode); staged (promise-form) nodes encode it
  // per-node and ignore it. Only segment-cache writes consume the decoded
  // partiality, so callers whose seeds are never written to the cache may
  // pass the conservative value (true).
  ...
  if (transportData !== null) {
    ...
    if (transportHead !== undefined) {
      ...
      // Navigation responses carry the boolean form. Per-segment prefetch
      // responses carry the promise form, whose status can be read
      // synchronously because those responses are fully buffered before they
      // are decoded — see the equivalent read in decodeTransportNode.
      isHeadPartial =
