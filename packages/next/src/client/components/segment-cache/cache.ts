 ...
 * response families currently diverge on keying and on the recorded tier,
 * so the write forks on `payloadFetchStrategy` below: non-null means this
 * is a payload of a per-segment prefetch response; null means a
...
function writeSegmentDataIntoCache(
  ...
  // recorded tier. Null for live-render responses, which take the
  // live-render branch below. See writeServerResponseIntoCache.
  payloadFetchStrategy: FetchStrategy.PPR | FetchStrategy.StaticShell | null,
  ...
  // Null when the response carries no verdict (`u`). Only the per-segment
  // branch consults it; a live-render response's entries record the
  ...
  if (payloadFetchStrategy !== null) {
    ...
    if (ownedEntry !== undefined && ownedEntry.status === EntryStatus.Pending) {
      ...
      upsertSegmentEntry(now, payloadVaryPath, fulfilledEntry, tree.varyPath)
    } else {
