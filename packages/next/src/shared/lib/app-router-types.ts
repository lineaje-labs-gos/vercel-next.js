...
/**
 * The response to a navigation or prefetch request. Produced by a live
 * render (navigations, refreshes, runtime prefetches, dynamic route tree
 * prefetches) or served from per-segment static storage (the `/_tree` route
 * tree prefetch and the per-segment prefetch responses generated by
 * collect-segment-data). Fields that only apply to some of these response
 * kinds are optional.
 */
export type NavigationFlightResponse = {
   ...
   * and the truncated initial payload forwards the page payload's; only
   * per-segment responses additionally promise-encode the head's partiality
   * (see TransportSegmentData). The page's runtime-data-access
   * flag (the page payload's `u`), forwarded from the staged decode of the
   * page data. Fulfilled `true` means a runtime prefetch would return more
