...
/// `existent_task` must never fabricate: opening a task that exists in neither memory nor
/// persistent storage (a stale reference to an already-collected or never-created task) must
/// **panic** rather than insert and return a blank `TaskStorage` (which would silently corrupt the
/// graph). A real, resident task must open fine.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn existent_task_panics_on_nonexistent_task() {
    let (tt, _persistence_dir) = create_tt("existent_task_panics_on_nonexistent_task");

    // A real, resident task: `existent_task` must succeed.
    let tt2 = tt.clone();
    ...
    // A synthetic non-transient id that was never created: `existent_task` must panic (not
    ...
    assert!(
        panicked,
        "existent_task on a never-created task must panic, not fabricate a blank task"
    );
