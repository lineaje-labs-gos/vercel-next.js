...
pub trait ExecuteContext<'e>: Sized {
    ...
    /// Like [`Self::task`], but for callers that **expect the task to already exist as a live
    /// task** (a read/lifecycle op on a task a live reference points at — e.g.
    /// `try_read_task_output`/`try_read_task_cell`, `task_execution_completed`).
    ///
    /// Unlike `task`, it must never fabricate: a task that exists in **neither memory nor
    /// persistent storage** — a stale reference to an already-collected or never-created task —
    /// **panics** instead of inserting and returning a blank (which would silently corrupt the
    /// graph). It additionally debug-asserts the task is not GC-soft-deleted: a `deleted` task was
    /// collected (edges scrubbed), so a read would return stale contents; every re-entry is
    /// supposed to funnel through `resurrect_deleted` at connect first, so hitting a read with the
    /// flag still set means a resurrection path was missed.
    ///
    /// Prefer this over `task` + a manual `!deleted()` assert wherever the task must already exist.
    /// (Creation/edge-follow sites — `get_or_create_task`, connect, aggregation edges, GC scrubs —
    /// must keep using `task`, which may legitimately create.)
    fn existent_task(&mut self, task_id: TaskId, category: TaskDataCategory)
    -> Self::TaskGuardImpl;
...
impl<'e> ExecuteContextImpl<'e> {
    ...
    /// Shared body of [`ExecuteContext::task`] and [`ExecuteContext::existent_task`]: opens a task,
    /// restoring the requested `category` from persistent storage if needed.
    ///
    /// When `require_existing` is false (the `task` path) a missing task is created — `access_mut`
    /// inserts a blank entry and, if disk has nothing, it stays empty. When `require_existing` is
    /// true (the `existent_task` path) a task that exists in **neither memory nor disk** is a bug
    /// (a stale reference to an already-collected/never-created task): this **panics** rather than
    /// fabricate and return a blank. Existence is judged from residency (a resident entry always
    /// exists) and the disk-presence flag `restore_task_data` returns for the categories we
    /// restore.
    fn open(
        &mut self,
        task_id: TaskId,
        ...
        // A resident entry always corresponds to a task that exists (only `task()` ever inserts a
        // blank, and only for a task being created). `require_existing` therefore only needs to
        // prove existence when the entry looks like a fresh blank: nothing restored, not a new
        // task. (A fully-evicted resident task also matches this shape, but it is on disk,
        // so the `found_on_disk` check below clears it — the panic fires only when the task
        // is in neither memory nor disk.)
        let mut task = self.backend.storage.access_mut(task_id);
        let maybe_fabricated = require_existing
        ...
        if !task.flags.is_restored(category) {
            if task_id.is_transient() {
                ...
                // A transient task is never persisted, so it can only exist in memory. A blank
                // entry means it does not exist.
                assert!(
                    !maybe_fabricated,
                    "existent_task({task_id}): transient task is not resident — it does not exist"
                );
            } else {
                ...
                if do_data || do_meta || data_restoring || meta_restoring {
                    ...
                    let found_on_disk = storage_data
                        .as_ref()
                        .is_some_and(|r| r.as_ref().map(|(_, found)| *found).unwrap_or(false))
                        || storage_meta
                            .as_ref()
                            .is_some_and(|r| r.as_ref().map(|(_, found)| *found).unwrap_or(false))
                        || data_restoring
                    ...
                    // The caller asserted this task exists, but it looked like a fresh blank and
                    // restore found nothing on disk (and no one else was restoring it): it exists
                    // nowhere. Fail loudly rather than hand back a fabricated task, which would
                    // silently corrupt the graph. (The leftover blank entry is inert; the panic
                    // tears the process down.)
                    assert!(
                        !(maybe_fabricated && !found_on_disk),
                        "existent_task({task_id}): task exists in neither memory nor persistent \
    ...
    /// Restores one category for a task from persistent storage. The `bool` reports whether the key
    /// was **present** on disk (`false` = absent, `storage` left empty). Normal `task()` restores
    /// ignore the flag (an absent key just yields an empty storage, as before); `existent_task`
    /// uses it to refuse to fabricate a task that exists nowhere.
    fn restore_task_data(
        &self,
        task_id: TaskId,
        category: SpecificTaskDataCategory,
    ) -> Result<(TaskStorage, bool)> {
        debug_assert!(
            ...
            _ => {
                match self.restore_task_data_batch(
                    ...
                    Ok(data) => {
                        ...
                        {
                            // Prefetch path: the disk-presence flag is unused here (only
                            // `existent_task` reads it), so pass a placeholder.
                            tasks[idx].data_restore_result = Some(Ok((item, true)));
                        }
                    }
                    Err(e) => {
            ...
            _ => {
                match self.restore_task_data_batch(
                    ...
                    Ok(data) => {
                        ...
                        {
                            // Prefetch path: disk-presence flag unused here; placeholder.
                            tasks[idx].meta_restore_result = Some(Ok((item, true)));
                        }
                    }
                    Err(e) => {
                        let msg = format!("{e:?}");
...
fn apply_restore_result(
    ...
    // `bool` is the disk-presence flag from `restore_task_data`; only `existent_task` acts on it,
    // so a normal restore ignores it here (an absent key applies as an empty storage, as before).
    result: Result<(TaskStorage, bool)>,
    category: SpecificTaskDataCategory,
) -> Result<()> {
...
impl<'e> ExecuteContext<'e> for ExecuteContextImpl<'e> {
    ...
    fn task(&mut self, task_id: TaskId, category: TaskDataCategory) -> Self::TaskGuardImpl {
        self.open(task_id, category, /* require_existing */ false)
    }
    ...
    fn existent_task(
        ...
        let task = self.open(task_id, category, /* require_existing */ true);
        // Beyond existence (enforced in `open`): a resident but GC-soft-deleted task means a
        // resurrection path was missed — reading it would return stale, edge-scrubbed contents.
        // Debug-only: the flag exists only when GC is enabled.
        debug_assert!(
            !task.deleted(),
            "existent_task({task_id}) opened a GC-deleted task — a resurrection path was missed"
        );
        task
    }
