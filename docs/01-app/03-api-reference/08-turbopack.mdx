...
| `turbopackMangleExportNames`                                                                                 | Shorten the export names modules expose to each other, to reduce bundle size. Recommended alongside minification with mangling; a module whose export names can be observed (an escaping namespace object, a dynamic `import()`, a CommonJS `require()`) keeps its original names. | `false`       | `true` on canary releases     |
